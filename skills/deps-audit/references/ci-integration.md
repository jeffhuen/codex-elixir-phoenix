...
1. **No interactive prompts.** User prompts would block forever
...
The `--ci` flag is designed to share the same exit-code rubric as the
planned `mix phx.deps_audit` Mix task shipped by the companion
`phx_deps_vet` Hex package. That keeps CI workflows portable between
interactive Codex use and teams that run the Mix task directly.

Until the companion package ships, do not gate CI on an interactive Codex
session. Run the audit locally through `$phx-deps-audit --ci ...` or defer
CI gating to the Mix task surface.
...
PostToolUse format hook in Codex warns on unformatted output —
