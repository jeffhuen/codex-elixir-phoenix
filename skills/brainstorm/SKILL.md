---
name: phx-brainstorm
description: "Use when exploring vague Elixir/Phoenix feature ideas, comparing approaches, gathering requirements, or deciding whether to plan."
---
...
**MANDATORY**: Write interview.md FIRST, then use ask the user directly.
...
3. Use ask the user directly with EXACTLY these options:

   - **Research** — search codebase + internet for approaches
...
**First cycle: MAX 2 research tracks** — keep it fast (~2-3 min). Delegate
only when Codex subagents are available and the user/task explicitly
authorizes delegation; otherwise run both tracks inline:

- Codebase patterns: "How does this codebase handle {topics}?"
  Write to `.claude/plans/{slug}/research/codebase-scan.md`
- External approaches: "Elixir/Phoenix approaches to {topics}"
  Return 500-word summary

**Do NOT run additional specialist tracks** in the first cycle.
If user wants deeper investigation, they pick "More research" at the
next Decision Point — then run focused tracks for specific questions.
...
Do NOT recommend one. Return to Decision Point (ask the user directly).

See `<skill-dir>/references/research-integration.md` for details.
...
5. **ask the user directly at EVERY decision point** — never flow past without formal choice.
   This is the most critical law. After interview, after research, after discuss — ALWAYS
   present options via ask the user directly. Never let conversation skip the checkpoint
6. **STOP after presenting options** — do not proceed without user input
7. **MAX 2 tracks in first research cycle** — deeper dives are subsequent cycles.
...
- `<skill-dir>/references/interview-techniques.md` — coverage scoring,
  question templates, scan patterns, signal detection, interview.md format
- `<skill-dir>/references/research-integration.md` — diverge-evaluate-converge,
  research-track templates, approach presentation format
