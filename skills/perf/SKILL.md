---
name: phx-perf
description: "Use when investigating Elixir/Phoenix slowness, timeouts, high memory, N+1 queries, assign bloat, Ecto optimization, or GenServer bottlenecks."
---
...
Run analysis tracks based on focus. Delegate tracks only when Codex subagents
are available and the user/task explicitly authorizes delegation; otherwise run
them inline.

**Ecto Track** (default or `--focus ecto`):

Analyze with this focus:
...
- `<skill-dir>/references/benchmarking.md` — Benchee patterns, profiling, flame graphs
