---
name: phx-deps-vet
description: "Use when approving a Hex package version after dependency audit findings, maintaining hex_vet.exs, seeding the ledger, or checking ledger drift."
---
...
1. **NEVER auto-approve.** Every entry MUST come from an `ask the user directly`
...
6. **Confirmation counts are COMPUTED, never estimated.** Any number in
   an `ask the user directly` (criteria split, new/overwrite/no-op) MUST be
...
- **`--seed`** → import `priv/hex_vet_seed.exs`. Before prompting,
  ...
  counts. Put those computed numbers in the `ask the user directly`. Also
...
Call `ask the user directly` with these 4 options:
...
- `<skill-dir>/references/hex-vet.md` — schema, parser, lookup
- `<skill-dir>/references/seed.md` — `--seed` flag, curated baseline
- `<skill-dir>/../deps-audit/references/rules-impl.md` — the
