...
description: "Use when the user corrects the approach, teaches a pattern, or a bug fix reveals a reusable Elixir/Phoenix lesson to capture."
---
...
- Grep project AGENTS.md for the pattern keyword
- Check auto-memory files for similar lessons
- Read `<skill-dir>/references/common-mistakes.md` (READ-ONLY plugin
...
`~/.codex/plugins/cache/` are cached and get overwritten on updates.
...
| All Elixir projects | Codex memory or user-level AGENTS.md guidance | "jsonb always uses string keys" |
...
**For durable personal memory** — use Codex memory when enabled, or write to
user-level guidance under the Codex home directory:
...
1. **NEVER edit plugin cache files** — files under `~/.codex/plugins/cache/` are cached and overwritten on updates
...
- `<skill-dir>/references/common-mistakes.md` — Common Elixir mistakes
