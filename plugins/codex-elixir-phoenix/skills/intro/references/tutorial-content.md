...
This plugin adds **specialist Elixir/Phoenix skills**, **context-aware guidance**, and **Iron Laws** to Codex. It turns a general-purpose AI into an opinionated Elixir pair programmer.
...
|---------|-------------|
| 48 skills | Workflow and domain guidance for every phase of development |
| 25 specialist agents | Ecto, LiveView, security, OTP, Oban, Ash, deployment review |
...
# 1. Plan — runs research tracks, outputs checkbox plan
...
# 3. Review — specialist tracks check idioms, security, tests, and
...
This runs specialist research tracks to analyze your existing plan and enhance it with research findings.
...
Iron Laws are non-negotiable rules that every relevant skill enforces. If your code violates one, Codex should stop and explain before proceeding.
...
| `$phx-audit` | 5-track project health audit with scores |
...
[Codex hooks](https://developers.openai.com/codex/hooks) run shell scripts automatically around tool use. These are real automation:
...
| Skill hints | Relevant tool events | Emits focused reminders when file paths or commands match Elixir/Phoenix work |
| Stop check | End of response | Warns about pending plan work or unverified Elixir changes |
...
before context compaction. This prevents rule loss when context is compressed.
...
This is **not hook infrastructure** — it's instruction-level routing that Codex follows alongside skill metadata.
This is the plugin's biggest known gap — hooks can emit reminders, but they do not directly load skills.
...
Running `$phx-init` injects enforcement rules **directly into your project's AGENTS.md**. This is stronger than plugin-level instructions because AGENTS.md is always read at session start.
...
The Iron Law Judge track does **pattern-based violation detection** — it uses search to inspect changed files for known anti-patterns. But it only runs when you invoke `$phx-review`.
...
AUTOMATIC (hooks):     Format check, security reminders, progress logging, failure hints,
                       Iron Laws for subagents when available, PreCompact rule preservation
...
STRENGTHENED BY:       $phx-init (injects rules into project AGENTS.md)
```
...
| `$phx-plan <feature>` | Plan with research tracks |
...
| `$phx-review` | Specialist track code review |
...
### Playing Nicely With Codex Built-Ins

The plugin complements Codex's built-in planning, shell execution, and plugin
support. A few patterns pair well with the Elixir workflow:

- Use `update_plan` during `$phx-work` or `$phx-full` so task progress stays visible.
- Use `$phx-permissions` when workspace roots, network access, or config profiles slow down Elixir work.
- Keep `.claude/plans/{slug}/scratchpad.md` and `progress.md` current; those files preserve checkbox progress, review findings, and handoffs across sessions.

### 3 Tips for Getting the Most Out of the Plugin

1. **Start with `$phx-plan` for any feature that touches multiple files.** The research tracks catch architectural issues early, before you've written code that needs rewriting.
