...
description: "Use when a Phoenix, LiveView, Ecto, Oban, test, bugfix, review, research, or refactor request has unclear intent or ambiguous scope."
---


# Intent Detection — Workflow Routing

When the user describes work without specifying a `$phx-*` skill or upstream
`/phx:*` command, analyze their intent and suggest the appropriate workflow
...
5. If user already specified a `$phx-*` skill or upstream `/phx:*` command:
   follow it, don't re-suggest
...
- SessionStart hooks (Tidewave/resume/project-state checks)
- AGENTS.md routing instructions (passive reference)
- Individual workflow skills (activated by explicit or implicit skill use)
