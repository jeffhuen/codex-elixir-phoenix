# LLM triage — two-tier consolidation
...
3. **TWO-TIER MANDATORY.** Per-package triage tracks write JSON files;
   a consolidation track summarizes. Main skill reads
...
deps-audit body
    ...
    │
    ├─► YES → run hex-deps-triager track for P
    │            ↓ writes triage/<pkg>-<ts>.json
    │
    ▼   (wait for all triage tracks)
context-supervisor track
    │
...
triage track fetches 3-line diff windows × N findings — easily 50KB
context per package. Running 10 packages in the main context and then
synthesizing exhausts the budget.

Splitting the work: per-package triage writes a small structured verdict
(1-3KB). The supervisor input is N × 3KB ≈ 30KB even at 10 packages.
Main skill reads a single consolidated.md (<5KB).

## Triager execution pattern

The skill body builds one input JSON per package, then runs triage tracks.
Delegate them only when Codex subagents are available and authorized;
otherwise process packages inline in descending score order:

```bash
for pkg in $(jq -r 'keys[]' high_score.json); do
  build_triager_input "${pkg}" > "/tmp/triage-in-${pkg}.json"
  run_or_delegate_triage "${pkg}" \
    "/tmp/triage-in-${pkg}.json" \
    ".claude/deps-audit/triage/${pkg}-$(date +%s).json"
done
wait
```

After all triage tracks complete, run the consolidation track:

```bash
consolidate_triage \
  ".claude/deps-audit/triage/*.json" \
  ".claude/deps-audit/triage/consolidated.md"
```
...
Validation pass after each triage output:
...
The consolidation track produces a markdown file shaped like:
...
### phoenix_extras 0.2.0 → 0.3.0 (score 22, model conf 0.92)
...
|-----------|--------|----------------------|
| Per-package triage input | ≤15KB | ~5-10KB |
| Per-package triage output | ≤3KB | ~1-2KB |
...
- No usable LLM/subagent capability available in the environment — fall back
