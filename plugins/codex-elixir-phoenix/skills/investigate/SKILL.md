...
description: "Use when investigating Elixir/Phoenix crashes, exceptions, stack traces, failing tests, regressions, data bugs, or unclear root causes."
---
...
Use **parallel mode** only when Codex subagents are available and the user/task
explicitly authorizes delegation. It is appropriate when:
...
**Avoid confirmatory delegation**: Do NOT spawn parallel subagents
...
present it directly. Do not spend extra context on four tracks to confirm
...
Parse the error message — check `<skill-dir>/references/error-patterns.md`.
...
- `<skill-dir>/references/error-patterns.md` — Common errors and checklist
- `<skill-dir>/references/investigation-template.md` — Output format
- `<skill-dir>/references/debug-commands.md` — Debug commands and common fixes
