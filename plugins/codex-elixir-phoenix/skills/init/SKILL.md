...
$phx-init --agents-only  # Install/update Codex custom agents only
```
...
4. **NEVER overwrite user-owned custom agents** — Only update `.codex/agents/*.toml` files that are missing or contain this plugin's generated header
...
```bash
<skill-dir>/../../tools/install-codex-agents.sh "$PWD"
```

If the installer is unavailable, copy manually using the rules below.

Rules:

1. Create `.codex/agents/` if needed.
2. For each source TOML file:
   - If the destination is missing, copy it.
   - If the destination contains `Generated by tools/generate-codex-agents.mjs`
     or `Converted from upstream agent source`, replace it with the latest source.
...
| `.codex/agents` read-only | Error: "Cannot install Codex custom agents - check permissions" |
| Existing custom agent conflict | Skip that file and report the path; never overwrite user-owned agent definitions |
