...
These rules govern ALL `$phx-*` workflow execution. Violations invalidate the session output.

1. **Skills are PROCEDURES, not suggestions.** Every numbered step MUST execute. Do not skip, reorder, or "optimize away" steps.
2. **Specialist tracks are MANDATORY when a skill says "always run".** Delegate only when Codex subagents are available and authorized; otherwise run the track inline.
...
| Security ≥ 4 | MANDATORY: minimum `$phx-plan` + security-analyzer track |
...
### STEP 6: RUN specialist tracks (MANDATORY when triggered)
