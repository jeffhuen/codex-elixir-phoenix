...
# Non-blocking. Stop hooks in Codex must emit JSON or no stdout.
INPUT=$(cat)
...
if [[ "$PENDING" -gt 0 ]]; then
  WARNINGS="${WARNINGS}${PENDING} plan(s) have uncompleted tasks"$'\n'
fi
...
if [[ "$BG_COUNT" =~ ^[0-9]+$ ]] && (( BG_COUNT > 0 )); then
  WARNINGS="${WARNINGS}${BG_COUNT} background task(s) still running — stop them or detach explicitly"$'\n'
fi
CRON_COUNT=$(echo "$INPUT" | jq -r '(.session_crons // []) | length' 2>/dev/null)
if [[ "$CRON_COUNT" =~ ^[0-9]+$ ]] && (( CRON_COUNT > 0 )); then
  WARNINGS="${WARNINGS}${CRON_COUNT} session cron(s) scheduled — see /schedule list"$'\n'
fi
